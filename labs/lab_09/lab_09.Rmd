...
top_10_per_capita <- clean_combined_pop %>%
  filter(loans_per_capita > 4450)
...
1. Start with the a subset of the WV ppp loan data we loaded earlier. **DONE**
2. Filter the data to examine only those loans less than $25,000. **DONE**
3. There are a bunch of NAs in the forgiveness amount column, for the purposes of this assignment only, let's assume those are actually supposed to be 0. So make all the NAs 0. **DONE**
