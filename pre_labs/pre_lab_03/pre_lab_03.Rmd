...
**Answer** There were not any parsing errors when I ran this codeblock with every row. 
...
**Answer** The amount field is a <dbl> data type. The name field is a <chr> data type. The id column is a <dbl> data type. 
```{r}
...
**Answer**: There are 192959 loans where the franchise_name is NA. Subway has 275 loans. 
...
**Answer**: There are 135073 loans where forgiveness_amount is NA.
...
**Answer**: There are 135073 loans where the forgiveness_date is NA. It is the same as the number for forgiveness_amount. 
...
**Answer**: This value is the same as the number for forgiveness_amount and forgiveness_date that was run separately in the previous codeblocks. 
...
**Answer** There are not any loans in the data for October 2020. This time frame was between the first and second PPP draws. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for $10,000,000. This number is not suspicious to me because I know this is a common number in the dataset and it is the maximum amount that a business can receive from the program. The minimum loan amount is $6. This number is slightly suspicious to me because it seems like it would be a huge hassle to fill out this application for a measly $6. I would want to see if this crazy small amount occurs often in the data. 
```{r}
...
**Answer** There are only two businesses that received a loan under $100.
```{r}
...
**Answer** All of the amounts fields do align. %>% 
...
**Answer** There are six rows in this demonstration data set. 
...
**Answer** The amount field is the <chr> data type.
```{r}
...
**Answer** Three problems that exist in this data that could prevent us from answering questions are the amount category being the wrong type of data, a row being fully duplicated, and the city field having five different forms, including misspellings, or Arnold. 
...
**Answer** This function changes the name of column "1_id" to "x1_id". 
...
**Answer** The error in this codeblock is that there is an invalid character type for the argument. This means that the character type for amount is incorrect and it needs to have a different type in order to be summarized. 
...
**Answer** The new data type for amount is <dbl>.
...
**Answer** There is a total of 2033179 loans in this data. 
```{r}
# cleaning function
total_arnold_md_loans <- cleaned_arnold_md_loans %>%
  summarise(total_amount = sum(amount))
...
**Answer** There are two duplicates in this data. The business name is Stuart Jones LLC.
...
**Answer** The new dataframe now has five rows instead of six.
...
**Answer** This code standardizes the ZIP codes by mutating them by the amount of characters they include. 
...
**Answer** There are two misspelling of Arnold left after running this code. 
...
**Answer** Only one misspelling of Arnold remains after running this code. 
...
**Answer** Zero misspellings of Arnold remain after running this code. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
