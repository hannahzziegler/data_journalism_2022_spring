...
https://datawrapper.dwcdn.net/HcyPq/1/ 
...
**Answer** The code creates a new dataframe, groups Maryland PPP loans by county and summarizes the number of loans allocated to each county. 
...
**Answer** The code changes the dataframe we originally created to get rid of the counties that are not in the state. We did this by filtering out any data from counties with less than 6 loans, all of which are not actually in Maryland. 
...
The FIPS codes are added for each county/census tract in the United States. After that, we filtered the dataframe to only have FIPS codes for counties in Maryland. Then, we joined the dataframe with FIPS codes for Maryland to the dataframe with PPP loans by county in the codeblock below. We combined the state_code and county_code columns in this new dataframe. 
...
The md_counties_with_fips dataframe joins the md_counties and md_fips dataframes by the county, so it now displays the loans per county and the FIPS for each county. 
...
**Answer** The map only has the state of Maryland's counties, rather than every county in the United States. 
...
**Answer** The hover caption for each county now has the county name, rather than the FIPS code for each county. It also adds commas to the loan amounts that are displayed. In my map, I also made the county names title case rather than uppercase. 
...
**Answer** https://datawrapper.dwcdn.net/pQox1/1/ 
