...
**Answer** There are 20 different sectors described and range in descriptiveness. For example, there is a part of the table devoted to "other services (except public administration), which is a very vague description. Information is also another category that seems overly broad. Some of the sectors also have two digit number separated by a hyphen, while others only have two digit numbers. 
...
**Answer** I see the full html code with head and body tags that is driving the display on this webpage. 
...
**Answer** The output that appears below the codeblock appears to have HTML code within it. It includes nested elements like head and body tags with classes within them. R is now holding the HTML page source within it.
...
**Answer** There are different subheadings with html and nested head and body tags with blue arrows next to them with dropdown menus. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** Under the table html class, the codeblock highlights the entire second table that we want to see in this prelab. 
...
**Answer** There are six different dataframes that R produces. Each one contains a table that was present on the original html page. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** There are six different dataframes that R produces with different data from each table housed on the html page. It includes tibblels with descriptions of each row and column. 
...
**Answer** The scroll icon produces a table with the data that corresponds to the format in the original List of 6 that was present before clicking. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** The table includes the information we are looking for of the 20 different sector codes and their descriptions, plus an extra row describing the source. 
...
**Answer** The table shows only the second table that appears on the html website. It looks like the data that was displayed in the original website with descriptions of each sector and the number that aligns with it. 
...
**Answer** The output standardizes the column sizes to make them more readable and removes the last row. It also standardizes the column headers. 
...
**Answer** The output includes a table tha tincludes the 20 sectors and descriptions from table #2 on original the html site. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It includes tables describing employment and other statistics relating to the classification. There are six different tables with information on this webpage. 

A few scrolls down the page, there's a table that has employee statistics.  

The table is called "Employment and Unemployment". There's a row in the tabled for "Employment, all employees (seasonally adjusted)".  And in that row, there's a value for the number of employees -- in thousands -- in November 2021.  
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** The output includes html code nested with head and body tags that describe the layout of the webpage. It is a list of 2 in the environment. 
...
**Answer** The code outputs the html element descriptions that correspond to the table that we want to see. It shows the tags related to the table with employment information. We used html element to filter for only the element that we want -- in this case the November 2021 employment data. 
...
**Answer** The codeblock below includes six columns with six rows. One is called "data series" with employment and unemployment information. The other column has backdata and is mostly empty. Every other column has the data for each of the months from the original table on the html page with information on employment and unemployment across four months. 
...
**Answer** The output that appears in this codeblock is the value that we wanted from the original data: just the employment of all employees for the month November 2021. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This page is similar to the utilities sector and includes many of the structures in the html page that appeared for that sector. It also includes the employment/unemployment data table. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This page is also very similar to the utilities and mining sectors. It includes many of the same structures/html layouts as the html page that was present for the utilities sector. It also includes the employment/unemployment data table.
...
**Answer** The output appears as each sector name standardized and ready to be used as column names in the dataframe. The output names each sector in quotation marks next to a [1]. 
```{r}
...
**Answer** It names a different row after each sector that we need to examine to create this dataframe. 
```{r}
...
**Answer** The code prints an error. The object "sector_list" is not found.
```{r}
...
**Answer** The output is the name of each sector in quotation marks, next to a [1].
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** The code prints an error because "sector_name" is not found. 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** The code prints the numbers 1-10 with a [1] before each one. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** The output displays a dataframe of each naics industry with the sector number and description. 
...
**Answer** The output in the codeblock is the number 20, which is the number of rows in the naics_industry dataframe. 
...
**Answer** This code assigns a number to each row within the naics_industry dataframe. 
...
**Answer** The output in the codeblock displays one row with two columns -- the employment and November 2021 employment rate. 
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** The output reads in all of the html from the table and stores all them. It filters to only keep the second dataframe on the original html page we were using, standardizes column headers and removes the last row. 
```{r}
# Define url of page we want to scrape
...
**Answer** The codeblock adds a column that has the url for each sector that we need that corresponds with the sector code. 
...
**Answer** The codeblock adds the tag to the html table that contains the information that we want to use from each page using the xpath.
...
**Answer** The codeblock deletes the row pertaining to Public Administration sector. Now, there are 19 rows in the dataframe. 
...
**Answer** The loop creates a separate dataframe for each sector that includes its sector number, description, url and xpath_id. 
...
**Answer** The code stores the url and xpath value. It iterates over each row in the naics industry dataframe and defines the url and id of the table to ingest. There us a url and an xpath for each row. 
```{r}
...
**Answer** The output gives a separate dataframe from each naics industry with the employment and unemployment data from the html page. Each dataframe contains all employment numbers from the original html table for each sector. 
...
**Answer** This table cleans up all of the previously made tables for each row to make sure they only show the November 2021 employment. The output also standardizes the columns names and uses slice to show only the row that we want to see. 
...
**Answer** The output adds columns that include the sector code and name. These descriptors are added to the previous table that we made that included the employment data from each month, as well as the xpath and url.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** The new dataframes now only include the naics sector number, description and November 2021 employment for each naics sector. 
...
**Answer** The end result is one single dataframe that includes all of the data that we need in our table. It includes the sector, description of the sector and the number of employees for November 2021. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** The page displays the workforce statistics for the mining sector. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** The data does not display as many workforce statistics and does not include any information on the employment, which is what we are looking for in this dataframe we are creating. It is structured differently than the other pages we have investigated. 
...
**Answer** This code removes the agriculture sector from the table. There are now 19 rows of employee data for each sector. 
