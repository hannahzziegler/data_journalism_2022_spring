...
**Answer** This data frame has 195865 rows and 63 columns. 
...
**Answer** The code is grouping the data by the county in which the project took place. Then, it gets the total number of loans for each county. Then it arranges these numbers in descending order and displays the new dataframe with the completed results. 
...
**Answer** There is a blank grey box without anything in it. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** There is a bar chart with the project county name on the x axis and the number of loans in the y axis. Each bar represents a different county, but the county names overlap with one another on the x axis. 
...
**Answer** The bars have been reordered based on the value of the x axis and appear in ascending order. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The y and x axes have been flipped so that the county name is on the y axis and the count of loans is on the x axis. Now, the county names are not as jumbled and hard to read. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Theme minimal adds the white lines that appear at each "count" block and makes the data easier to read. 
...
**Answer** The code makes this chart appear as if it is in The Economist, with a light blue background, dark grey bars and separate white lines that separate each part of the y axis. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The text has better spacing and a bolded title that describes the graph. The name of the y axis was not descriptive to the data before but now it is easier to read for the audience. It also adds a caption that attributes the data somewhere. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code has a summary of the total loans by the date they were approved. It is not arranged in any particular order. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code has a line chart with total loans on the y axis and the approval date on the x axis. The date approved values on the x axis are also very spread out, as are the total loans values, which makes the graph difficult to read. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The scale for the y axis has changed to have each month using the scale_x_date code. Now, each month is displayed on the y axis and it is very squished. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Element text allowed us to adjust the text to the axis that we are working on. Now all of the months are displayed clearly at an angle and justification relative to the tick marker associated with the month. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
