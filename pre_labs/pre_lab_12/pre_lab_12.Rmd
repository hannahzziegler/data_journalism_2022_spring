...
**Answer** The code below displays a subtitle that prompts the chart to automatically update the subtitle in the graph depending on whether the vaccinations in the state increased or decreased. Paste combines the text of our subtitle with the data that will be updated each day. 
...
**Task** Run the following code and put the value of `sub` in the answer.
**Answer** The code adds a subtitle to the bar chart so that the chart can be labeled. The chart displays the original data with a blue trend line that is overlaid on top of it to show the trend. It also shows the subtitle that will update itself automatically each time that we update the vaccination totals. 
