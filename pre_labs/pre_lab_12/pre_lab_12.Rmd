...
**Answer** The dataframe has 11397 rows and 13 columns. 
...
**Answer** The dataframe has each Maryland county and the number of days that each county has vaccination data for. There is an extra row in the county column that displays the county name as "NA" and an extra row called "unknown," which is unusual. 
...
**Answer** The NA records represent vaccinations where they do not know which county a person was vaccinated in. It could also represent that the government did not know where the people being vaccinated were from, but that is less likely than the former hypothesis. The dataframe below doesn't pertain to a county because the numbers are too strange. 
...
**Answer** In the last 7-10 days, it seems like the number of vaccinations in the county has steadily decreased and the numbers ave been in the 320-360 range, which is near the lowest amount of daily vaccinations, as the rankings only go until 364 when sorted in that way. 
...
**Answer** The chart shows a trend of skyrocketing vaccinations in early to mid-2021 and then a slow petering off of the vaccination counts throughout the rest of 2021. The vaccinations remain relatively consistent throughout the rest of the data displayed, with a slight jump in vaccinations around December 2021.
...
**Answer** The code below displays a subtitle that prompts the chart to automatically update the subtitle in the graph depending on whether the vaccinations in the state increased or decreased. Paste combines the text of our subtitle with the data that will be updated each day. The value of 'sub' is [1] "The number of people in Prince George's County who got fully vaccinated on March 06 decreased by -73.77 percent compared to the week before"
...
**Task** Run the following code and put the value of `sub` in the answer.
**Answer** The code adds a subtitle to the bar chart so that the chart can be labeled. The chart displays the original data with a blue trend line that is overlaid on top of it to show the trend. It also shows the subtitle that will update itself automatically each time that we update the vaccination totals. The value of 'sub' is [1] "The number of people in Prince George's County who got fully vaccinated on March 06 decreased by -73.77 percent compared to the week before"
...
**Answer** The resulting line graph shows the case total per county per day on a scale of every six months starting in July of 2020. 
...
**Task** Run the following code and describe what the resulting chart shows
**Answer** The chart shows the same data but with a log scale and using scientific notation to display the number of cases. This graph makes it seem like the pandemic was much worst from the start of July 2020. 
...
**Answer** There are 3233 rows in the dataframe and 9 columns. 
...
**Answer** The code below filters to find only the data for counties in Maryland.
...
**Answer** The map appears to show the cumulative cases of COVID-19 in the entire state county-by-county. The lighter the color displayed on the map, the more cases that county has experienced. 
