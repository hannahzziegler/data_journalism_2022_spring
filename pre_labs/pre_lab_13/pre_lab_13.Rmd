...
**Answer** There is a dataframe with five columns and 51 rows describing the percentage of vote that Trump received in the state and the percentage of the state's small businesses that received a PPP loan, as well as other variables. 
```{r}
...
**Answer** The plot shows a weaker positive regression line, but the points are not aligned in a very linear way. The line suggests that there is some form of relationship between the businesses receiving PPP loans and the percent of the population that voted for Trump. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The plot has a weaker positive regression line showing the relationship between the amount of PPP loan funding per employee and the percentage of the vote that went to Trump in each state. 
```{r}
...
**Answer** The table outputs a moderate .52 correlation between businesses receiving PPP loans and the percentage of their population that voted for Trump. The table also shows a weak .22 correlation between the PPP loan amount per employee and how much of the population voted for Trump. 
```{r}
...
**Answer** The output gives a p value of 0.00008607 at a 95 percent confidence interval. This p value is less than .05, which means that the result is statistically significant.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** This output gives a p-value of 0.1189 and t value of 1.5872. This p value is greater than .05, which means that it is not statistically significant. 
```{r}
...
**Answer** The code displays a table with the average PPP loan amount per employee and the mean percent of businesses receiving PPP loans in a comparison of Clinton and Trump voters in 2016. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The two sample t-test returns a t value of 1.2223 and a p value of 0.2295. The t value is low, the p value is pretty high, and there are not a lot of meaningful correlational results from analysis of this data. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The t value is much stronger at about 3 and the p value is .002. The p value is less than .05 and t value is high, which means that the results of this analysis could be statistically significant. 
```{r}
